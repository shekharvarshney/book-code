...
public class TradeInfoServiceTest {
	...
	public void accountsForInstrumentsTest() {
		TradeInfoService<Long, String, Long> service = createService();
		Collection<Long> accountIds = service.findAllAccountsWithInstrumentTrades(new TradeableInstrument<String>(
				"EUR_USD"));
	...
	private TradeInfoService<Long, String, Long> createService() {
		...
		AccountInfoService<Long, String> accountInfoService = mock(AccountInfoService.class);
		TradeInfoService<Long, String, Long> service = new TradeInfoService<Long, String, Long>(
		...
		when(accountInfoService.getAllAccounts()).thenReturn(Lists.newArrayList(account1, account2));
		when(tradeManagementProvider.getTradesForAccount(TradingTestConstants.accountId)).thenReturn(
				createSampleTrades1());
		when(tradeManagementProvider.getTradesForAccount(TradingTestConstants.accountId2)).thenReturn(
				createSampleTrades2());
		service.init();
		return service;
	}
	...
	private Collection<Trade<Long, String, Long>> createSampleTrades1() {
		...
		trades.add(new Trade<Long, String, Long>(2001L, 10, TradingSignal.LONG, new TradeableInstrument<String>(
				"GBP_USD"), DateTime.now(), 0.0, 1.5365, 0.0, TradingTestConstants.accountId));
		trades.add(new Trade<Long, String, Long>(2003L, 10, TradingSignal.LONG, new TradeableInstrument<String>(
				"USD_JPY"), DateTime.now(), 0.0, 120.15, 0.0, TradingTestConstants.accountId));
		trades.add(new Trade<Long, String, Long>(2005L, 10, TradingSignal.SHORT, new TradeableInstrument<String>(
				"EUR_USD"), DateTime.now(), 0.0, 1.2429, 0.0, TradingTestConstants.accountId));
		return trades;
	}
	...
	private Collection<Trade<Long, String, Long>> createSampleTrades2() {
		...
		trades.add(new Trade<Long, String, Long>(2002L, 10, TradingSignal.SHORT, new TradeableInstrument<String>(
				"EUR_JPY"), DateTime.now(), 0.0, 135.55, 0.0, TradingTestConstants.accountId2));
		trades.add(new Trade<Long, String, Long>(2004L, 10, TradingSignal.SHORT, new TradeableInstrument<String>(
				"GBP_NZD"), DateTime.now(), 0.0, 2.39, 0.0, TradingTestConstants.accountId2));
		trades.add(new Trade<Long, String, Long>(2006L, 10, TradingSignal.SHORT, new TradeableInstrument<String>(
				"AUD_USD"), DateTime.now(), 0.0, 0.8123, 0.0, TradingTestConstants.accountId2));
		trades.add(new Trade<Long, String, Long>(2007L, 10, TradingSignal.LONG, new TradeableInstrument<String>(
				"EUR_USD"), DateTime.now(), 0.0, 1.2515, 0.0, TradingTestConstants.accountId2));
		return trades;
	}
}
